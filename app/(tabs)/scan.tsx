...
export default function ScanScreen() {
  ...
  const qrPayload = JSON.stringify({ userId: user?.id || 'guest' });
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      // If we scan another user's QR, we would add them as a friend.
      // Currently, we just mock the success of adding a friend.
      if (data.includes('userId')) {
        setResult({ success: true, message: 'Friend added successfully!' });
      } else {
